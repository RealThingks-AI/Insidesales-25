  ...
  return useQuery({
    ...
    queryFn: async (): Promise<DashboardData> => {
      ...
      const [dealsRes, accountsRes, contactsRes, actionItemsRes, emailRes, todayRes, activityRes] = await Promise.all([
        ...
        // Today's agenda
        supabase.from('action_items').select('id, title, priority, due_time, module_type').eq('assigned_to', user.id).eq('due_date', new Date().toISOString().split('T')[0]).neq('status', 'Completed').neq('status', 'Deferred').order('due_time', { ascending: true }).limit(10),.select('id, title, priority, due_time, module_type').eq('assigned_to', user.id).eq('due_date', new Date().toISOString().split('T')[0]).neq('status', 'Completed').neq('status', 'Deferred').order('due_time', { ascending: true }).limit(10),.order('due_time', { ascending: true }).limit(10),
        // Recent activities
